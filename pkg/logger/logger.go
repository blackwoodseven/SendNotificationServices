...
import (
	...
	"github.com/rajivganesamoorthy-kantar/SendNotificationServices/pkg/logger/models"
)
...
func Log(group string, severity SeverityStatus, msg string, err error, val interfaces.LogValue) {
	mu.Lock()
	defer func() {
		mu.Unlock()
	}()
...
func logContextToSystem(msg string, err error) {
	...
	sysLog := models.SystemContextErrorLog{
		Time:               getLogTime(),
		SeverityStatusName: SeverityError.statusName(),
		SeverityStatusNo:   SeverityError.statusNumber(),
		ErrorMsg:           msg,
		Error:              marshalledErr,
		Stack:              getStack(),
	}
...
func logSystem(group string, severity SeverityStatus, msg string, err error, val interfaces.LogValue) {
	var bSysLog []byte
